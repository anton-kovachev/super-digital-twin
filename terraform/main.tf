...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      CORS_ORIGINS     = "https://${aws_apigatewayv2_api.main.api_endpoint}" 
      S3_BUCKET        = aws_s3_bucket.memory.id
      USE_S3           = var.use_s3
      BEDROCK_MODEL_ID       = var.bedrock_model_id
      AWS_BEDROCK_BASE_URL   = var.aws_bedrock_base_url
      ...
      GITHUB_MCP_TOKEN = var.github_mcp_token
      LLM_MODEL_ID = var.llm_model_id
...
resource "aws_apigatewayv2_route" "get_greeting" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "GET /greeting"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "get_sample_prompts" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "GET /sample-prompts"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "get_cv" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "GET /cv"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
...
resource "aws_route53_record" "alias_root" {
  ...
  alias {
    ...
    zone_id                = local.s3_website_zone_ids[var.aws_region]
